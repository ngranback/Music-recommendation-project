...
class PgConnection {
   ...
   */
  getAllArtists(tableName) {
    const sql = this.sqlUtil.getAll(tableName);
    return this.query(sql);
  }

  getAllUserArtists(tableName) {
    const sql = this.sqlUtil.getAll(tableName);
    return this.query(sql);
  }

  getAllSimilarArtists(tableName) {
    const sql = this.sqlUtil.getAll(tableName);
    return this.query(sql);
  }
   ...
   */
   ...
   async getById(tableName, requested_artist) {
    const sql = this.sqlUtil.getById(tableName, requested_artist);
    const rows = await this.query(sql, requested_artist);
